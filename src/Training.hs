...
train' params net e = do
  let nextNet = trainEpoch params net
  putStrLn $ "Epoch " ++ show ((params ^. epochs) - e + 1) ++ ":"
  train' params nextNet $ e - 1


trainEpoch :: TrainParams -> Network -> Network
trainEpoch params net = foldl (trainBatch params) net batches
...
trainBatch :: TrainParams -> Network -> [([Double],[Double])] -> Network 
trainBatch params net batch = net & layers .~ updatedDenseLyrs 
  where gradients = processBatch net batch  
...
processBatch :: Network -> [([Double],[Double])] -> [Gradient] 
processBatch net batch = map normalize sumGrads
  where normalize g = g & dw %~ smul (1 / fromIntegral (length batch)) 
        ...
        step (x, t) = backprop (feedForward $ newInputNet [x] net) [t] 
        gradients = map step batch
